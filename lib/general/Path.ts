...
export class Path implements Transformable, IterableIterator<Point>, Cloneable {
     ...
     * // (1,1)->(2,2)->(3,3)->(4,4)
     */
    add(point: Point, pos: number = this.size): this {
        
        if(pos >= this.size) this._points.push(point);

        else if (pos <= 0) this._points.unshift(point);
